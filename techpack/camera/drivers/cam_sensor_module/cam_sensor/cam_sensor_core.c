...
extern struct cam_sensor_id_stats cam_sensor_id_stats_front;
extern struct cam_sensor_id_stats cam_sensor_id_stats_rear;
//-P86801AA1 huangxiaofeng.wt,ADD,2023/08/01, Added the status of the front camera, fixed the issue of using front and rear shots together
...
int cam_sensor_match_id(struct cam_sensor_ctrl_t *s_ctrl)
{
	...
	//+bug P86801AA1-1797, qinduilin.wt, ADD, 20230427, P86801AA1 camera bringup.
	if (chipid == 0x4608) {
		hardwareinfo_set_prop(HARDWARE_BACK_CAM,"HI846-TRULY");
		hardwareinfo_set_prop(HARDWARE_BACK_CAM_MOUDULE_ID,"TRULY");
	} else if (chipid == 0x0803) {
		hardwareinfo_set_prop(HARDWARE_BACK_CAM,"C8496-SHENGTAI");
		hardwareinfo_set_prop(HARDWARE_BACK_CAM_MOUDULE_ID,"SHENGTAI");
	} else if (chipid == 0x08a3) {
		hardwareinfo_set_prop(HARDWARE_BACK_CAM,"GC08A3-CXT");
		hardwareinfo_set_prop(HARDWARE_BACK_CAM_MOUDULE_ID,"CXT");
	} else if (chipid == 0x0556) {
		hardwareinfo_set_prop(HARDWARE_FRONT_CAM,"HI556-LCE");
		hardwareinfo_set_prop(HARDWARE_FRONT_CAM_MOUDULE_ID,"LCE");
	} else if (chipid == 0x0503) {
		hardwareinfo_set_prop(HARDWARE_FRONT_CAM,"C5590-TXD");
...
int cam_sensor_power_up(struct cam_sensor_ctrl_t *s_ctrl)
{
	...
	//+P86801AA1 huangxiaofeng.wt,ADD,2023/08/03, Added the status of the front camera, fixed the issue of using front and rear shots together
	...
	if(temp_id == CAMERA_REAR_W11_HI846 ||
		temp_id == CAMERA_REAR_W11_c8496 ||
		temp_id == CAMERA_REAR_W11_GC08A3){
		cam_sensor_id_stats_rear.cam_id = temp_id;
		cam_sensor_id_stats_rear.cam_stats = CAM_STATS_TURE;
	}
	//-P86801AA1 huangxiaofeng.wt,ADD,2023/08/03, Added the status of the front camera, fixed the issue of using front and rear shots together
...
int cam_sensor_power_down(struct cam_sensor_ctrl_t *s_ctrl)
{
	...
	CAM_INFO(CAM_SENSOR, "power_down_front cam_stats:%d,voltage_front:%d power_down_rear_status:%d,voltage_rear:%d,",
	cam_sensor_id_stats_front.cam_stats,cam_sensor_id_stats_front.last_voltage,cam_sensor_id_stats_rear.cam_stats,cam_sensor_id_stats_rear.last_voltage);
	//-P86801AA1 huangxiaofeng.wt,ADD,2023/08/01, Added the status of the front camera, fixed the issue of using front and rear shots together
