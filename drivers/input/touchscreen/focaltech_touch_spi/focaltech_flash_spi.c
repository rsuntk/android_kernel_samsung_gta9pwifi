...
//+P86801AA1, peiyuexiang.wt, 20230426, add, hardware_info
...
#define FTS_FW_NAME_PREX_WITH_REQUEST               "focaltech_ft8203_fw_"
...
//+P86801AA1 peiyuexiang.wt 20230426, add, hardware_info
#define FT8203_IC_CHIP_ID   0x82
#define FT8203_DJN_VENDOR_ID    0xda
#define FT8203_DSBJ_VENDOR_ID   0x84

static  u8 tp_vendor_id = 0;
static  u8 tp_ic_type = 0;
static  u8 firmware_version_from_ic = 0;

static void fts_read_tp_hardware_info(void)
{
    ...
    FTS_ERROR("factory read vendor id :0x%02x, ic_type :0x%02x, firmware_version_from_ic: 0x%02x",\
...
static void fts_hardwareinfo_set(void)
{
    ...
    if(tp_vendor_id == FT8203_DJN_VENDOR_ID)
    {
        snprintf(vendor_id,HARDWARE_MAX_ITEM_LONGTH,"BOE");
    } else if (tp_vendor_id == FT8203_DSBJ_VENDOR_ID) {
		snprintf(vendor_id,HARDWARE_MAX_ITEM_LONGTH,"DSBJ");
	} else {
        snprintf(vendor_id,HARDWARE_MAX_ITEM_LONGTH,"Other vendor");
    }

    if(tp_ic_type == FT8203_IC_CHIP_ID)
    {
        snprintf(ic_name,HARDWARE_MAX_ITEM_LONGTH,"FT8203");
    }else{
        snprintf(ic_name,HARDWARE_MAX_ITEM_LONGTH,"Other IC");
    }

    snprintf(TP_name,HARDWARE_MAX_ITEM_LONGTH,"%s,VND:%s,FW:0x%02x",
				ic_name,vendor_id,firmware_version_from_ic);
    FTS_INFO("firmware_ver=%s", TP_name);
}
//-P86801AA1 peiyuexiang.wt 20230426, add, hardware_info
...
{
#ifdef CONFIG_QGKI_BUILD//(LINUX_VERSION_CODE < KERNEL_VERSION(5, 10, 0))
    int ret = 0;
...
static int fts_fw_resume(bool need_reset, enum FW_TYPE fw_type)
{
    ...
    int i = 0;
    u32 app_off = 0;
    bool irq_need_recovery = 0;
    bool esd_need_recovery = 0;
    ...
    for (i = 0; i < 3; i++) {
        FTS_INFO("fw download times:%d", i + 1);
        ret = fts_fw_write_start(fw_buf + app_off, fwlen - app_off, need_reset);
        if (0 == ret)
            break;
    }
    if (i >= 3) {
...
static int fts_fwupg_get_module_info(struct fts_upgrade *upg)
{
    ...
    if (strstr(Lcm_name_tp,"focaltech_ft8203_wt_dsi_vdo_90hz_apf_boe")) {
        info = &module_list[0];
	}
    if (strstr(Lcm_name_tp,"ft8203_wt_dsi_vdo_90hz_focaltech")) {
        info = &module_list[0];
    }
    if (strstr(Lcm_name_tp,"dsbj_ft8203_wt_dsi_vdo_90hz_inx")) {
        info = &module_list[1];
    }
